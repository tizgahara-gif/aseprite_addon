...
function command.run(ctx)
  ...
  dlg:check{ id = "auto_sync_default", label = "Auto Sync", selected = ctx.config.get("auto_sync_default") }
  dlg:entry{ id = "debounce_seconds", label = "Debounce Sec", text = tostring(ctx.config.get("debounce_seconds") or 0.8) }
  dlg:entry{ id = "relay_url", label = "Relay URL", text = ctx.config.get("relay_url") or "" }
  dlg:check{ id = "deflate_enabled", label = "Deflate", selected = ctx.config.get("deflate_enabled") }
  ...
  dlg:check{ id = "show_sync_status", label = "Show Sync Status", selected = ctx.config.get("show_sync_status") }
  ...
  ctx.config.set("auto_sync_default", d.auto_sync_default)
  ctx.config.set("debounce_seconds", tonumber(d.debounce_seconds) or ctx.config.get("debounce_seconds"))
  ctx.config.set("relay_url", d.relay_url or "")
  ctx.config.set("deflate_enabled", d.deflate_enabled)
  ...
  ctx.config.set("show_sync_status", d.show_sync_status)
